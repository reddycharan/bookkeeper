...
public class BookKeeperDiskSpaceWeightedLedgerPlacementTest extends BookKeeperClusterTestCase {
     ...
     */
    @Test(timeout=60000)
    public void testDiskSpaceWeightedBookieSlection() throws Exception {
        ...
        // since the number of ledgers created is small (5000), we allow a range of 2X to 4X instead of the exact 3X
     ...
     */
    @Test(timeout=60000)
    public void testDiskSpaceWeightedBookieSlectionWithChangingWeights() throws Exception {
        long freeDiskSpace=1000000L;
        int multiple=3;
        for (int i=0; i < numBookies; i++) {
            // the first 8 bookies have freeDiskSpace of 1MB; While the remaining 2 have 3MB
        ...
        Thread.sleep(100);
        for (int i = 0; i < 5000; i++) {
            LedgerHandle lh = client.createLedger(1, 1, DigestType.CRC32, "testPasswd".getBytes());
            lh.addEntry("000".getBytes());
            for (BookieSocketAddress b : lh.getLedgerMetadata().getEnsemble(0)) {
                m.put(b, m.get(b)+1);
            }
     ...
     */
    @Test(timeout=60000)
    public void testDiskSpaceWeightedBookieSlectionWithBookiesDying() throws Exception {
        long freeDiskSpace=1000000L;
        int multiple=3;
        for (int i=0; i < numBookies; i++) {
        ...
        // since the number of ledgers is small (5000), there may be variation 
        ...
        for (int i=0; i < numBookies-3; i++) {
            ...
            assertTrue("Weigheted placement is not honored: " + delta, delta <= 25); // the deviation should be less than 25%
        }
     ...
     */
    @Test(timeout=60000)
    public void testDiskSpaceWeightedBookieSlectionWithBookiesBeingAdded() throws Exception {
        long freeDiskSpace=1000000L;
        int multiple=3;
        for (int i=0; i < numBookies; i++) {
     ...
     */
    @Test(timeout=60000)
    public void testDiskSpaceWeightedBookieSlectionWithPeriodicBookieInfoUpdate() throws Exception {
        long freeDiskSpace=1000000L;
        int multiple=3;
        for (int i=0; i < numBookies; i++) {
        ...
        for (int i=0; i < bookiesToCheck; i++) {
            double delta = Math.abs((double)m.get(bs.get(i).getLocalAddress())-(double)m.get(bs.get(i+1).getLocalAddress()));
            delta = (delta*100)/(double)m.get(bs.get(i+1).getLocalAddress());
            assertTrue("Weigheted placement is not honored: " + delta, delta <= 25); // the deviation should be <25%
        }
