...
public class EntryLogTest {
    ...
    class LedgerStorageFlushTask implements Callable<Boolean> {
        LedgerStorage ledgerStorage;
        ...
        public Boolean call() {
            try {
                ...
                LOG.error("Got Exception while trying to flush LedgerStorage");
                return false;
            }
            return true;
        }
    ...
    /**
     * test concurrent write and flush operations and then concurrent read
    ...
    public void testConcurrentWriteAndReadCallsOfInterleavedLedgerStorage() throws Exception {
        ...
        // add flush tasks also
        for (int i = 0; i < writeAndFlushTasks.size() / 100; i++) {
            writeAndFlushTasks.add(new LedgerStorageFlushTask(ledgerStorage));
        }
