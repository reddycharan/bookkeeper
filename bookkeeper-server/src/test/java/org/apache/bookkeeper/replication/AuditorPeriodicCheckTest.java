...
public class AuditorPeriodicCheckTest extends BookKeeperClusterTestCase {
    ...
    /*
     * Shows how writes that fail on one of the write quorum nodes is fixed by the
    ...
    public void testFailedWriteRecovery() throws Exception {
        ...
        for (int i = 0; i < numBookies; i++) {
            ReplicationWorker rw = new ReplicationWorker(zkc, baseConf, bs.get(i).getLocalAddress());
            rw.start();
            l.add(rw);
        }
        underReplicationManager.close();
