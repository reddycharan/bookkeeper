    ...
    public static boolean nukeExistingCluster(ClientConfiguration conf, String ledgersRootPath, String instanceId,
        ...
        try {
            ...
            if (!confLedgersRootPath.equals(ledgersRootPath)) {
                LOG.error(
                        "Provided ledgerRootPath : {} is not matching with config's ledgerRootPath: {}, so exiting nuke operation",
                        ledgersRootPath, confLedgersRootPath);
                return false;
            }
            ...
            if (!ledgerRootExists) {
                LOG.info(
                        "There is no existing cluster with ledgersRootPath: {} in ZKServers: {}, so exiting nuke operation",
                        ledgersRootPath, conf.getZkServers());
                return true;
            }
            ...
            if (availableNodeExists) {
                Collection<BookieSocketAddress> rwBookies = BookieWatcher.getBookies(zkc, availableBookiesPath);
                if (rwBookies != null && !rwBookies.isEmpty()) {
                    LOG.error(
                            "Bookies are still up and connected to this cluster, stop all bookies before nuking the cluster");
                    return false;
                }
                ...
                if (readonlyNodeExists) {
                    Collection<BookieSocketAddress> roBookies = BookieWatcher.getReadOnlyBookies(zkc,
                            readOnlyBookieRegPath);
                    if (roBookies != null && !roBookies.isEmpty()) {
                        LOG.error(
                                "Readonly Bookies are still up and connected to this cluster, stop all bookies before nuking the cluster");
                        return false;
                    }
                }
            }
